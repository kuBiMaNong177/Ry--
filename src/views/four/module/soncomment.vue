...
export default {
  ...
  created() {
    ...
    console.log(this.id)
     this.axios
      .get("http://localhost:3000/comment/event?threadId="+this.id)
      .then(res => {
        console.log(res.data.comments);
        this.list = res.data.comments;
        this.quantity=res.data.comments.length
      });
      // this.axios.get('http://localhost:3000/comment?t=1&type=6&threadId='+this.id+"&content="+this.$refs.buttn.value)
      // .then(res=>{
      //   console.log(res)
      // })
  }
